...
ui <- shinyUI(fluidPage(
  ...
  navlistPanel(
    ...
    tabPanel(
      "Upload Files",
      textInput("id", "Patient number:", value = "Patient 15A", width = NULL, placeholder = "Patient 15A"),
...
server <- shinyServer(function(input, output) {
  ...
  # Display example datasets: Patient 15A
  output$Example1<- DT::renderDataTable({
    eg1<- read.csv('patient15A.csv')
    DT::datatable(eg1)
  })
  
  output$Example2<- DT::renderDataTable({
    eg2<- read.csv('patient15A_malperfused.csv')
    DT::datatable(eg2)
  })
  ...
  output$barchart<- renderPlotly({
    # Clean dateset
    df1 <- table1()
    df1[1,c(2:3)] <- df1[1,c(2:3)]+df1[8,c(2:3)] #merge femoral head bone and metaphyseal bone
    df1 <- df1[1:7,]
...
","Malperfusion - Dark on post")
    ...
    # eparate tissue 2 from malperfusion 
    tissue2=df1[7,]
...
","Malperfusion - Dark on post","Tissue 2 - Dark T1, Dark T1 fs, Dark post") )+ 
  ...
  output$pie1<- renderPlotly({
    ...
    # Clean dataset
    df1 <- table1()
    df1[1,c(2:3)] <- df1[1,c(2:3)]+df1[8,c(2:3)]
    df1 <- df1[1:7,]
    df1 <- df1 %>% arrange(factor(Segment,levels=c("Bone - Bright on T1", "Epiphyseal Cartilage - Bright on T1 fs", "Physeal Cartilage - Bright on T1 fs", "Epiphyseal Tissue 1 - Grey T1, Grey T1 fs, White post","Physeal Tissue 1 - Grey T1, Grey T1 fs, White post","Malperfusion - Dark on post","Tissue 2 - Dark T1, Dark T1 fs, Dark post") ))
    Segment=c("Bone - Bright on T1", "Epiphyseal Cartilage - Bright on T1 fs", "Physeal Cartilage - Bright on T1 fs", "Epiphyseal Tissue 1 - Grey T1, Grey T1 fs, White post","Physeal Tissue 1 - Grey T1, Grey T1 fs, White post
","Malperfusion - Dark on post")
...
","Malperfusion - Dark on post")
    ...
    seg_perthes <- df1[c(1:5,7),c(1,3)]
    malperfusion = df1$VolumePerthes[6]
    #perfused = sum(seg_perthes$VolumePerthes)-malperfusion
    malperfusedTissue <- data.table(Segment=c('Tissue 2','Malperfused Bone','Epiphyseal Cartilage Overlapped with Malperfusion','Physeal Cartilage Overlapped with Malperfusion'), Volume=c(df2$VolumePerthes))
